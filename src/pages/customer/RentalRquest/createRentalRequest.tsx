...
interface CreateRentalRequestContentProps {
  ...
  rentalId?: number; // ✅ optional edit mode
}
...
const CreateRentalRequestContent: React.FC<CreateRentalRequestContentProps> = ({ onBack, onNextStep, rentalId }) => {
  ...
  // ward is the last segment, street is the rest joined back
...
const handleSaveDraft = async () => {
  ...
  const body: any = {
    id: rentalId,                        // required for update
    eventName,
    phoneNumber,
    email,
    description,
    ...
    status: "Draft",
    accountId: 1, // TODO: use real logged-in user later
    eventActivityId: selectedActivityId,
    activityTypeId: selectedTypeId,
  };

  // Only add createdDate for new rentals
  if (!rentalId) {
  ...
  try {
    ...
    // Return ID (important for Next Step)
  ...
  // Load types when activity changes
  useEffect(() => {
    ...
    (async () => {
      ...
      // keep current selectedTypeId if still valid; otherwise clear
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setWards(res || []);
      // clear ward if not belonging to province
  ...
  // ---------- EDIT MODE: load rental if rentalId is provided ----------
  useEffect(() => {
    if (!rentalId) return;
    (async () => {
      try {
...
// eventDate from API → input[type=date] expects YYYY-MM-DD
...
// parse address into street + wardName
...
// Province by name match + Ward by name match
if (r.city) {
  // 1) ensure provinces loaded and select province by name
  ...
  if (province) {
    ...
    // 2) ensure wards for that province are available, then select ward by name
    //    (your other effect also loads all wards; this is deterministic & immediate)
    let wardList = wards;
  ...
  }, [rentalId, provinces.length]); // re-run once provinces are available
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              disabled={!selectedActivityId} // ✅ disabled until activity selected
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              disabled={!selectedProvinceId} // ✅ disabled until province selected
