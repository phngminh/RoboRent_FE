...
interface CreateRentalDetailContentProps {
  ...
  rentalId: number;         // must be provided (you saved rental first)
  activityTypeId: number;   // from CreateRentalRequest step (selectedTypeId)
  onSave: () => void;
}
...
useEffect(() => {
  ...
  (async () => {
    ...
    try {
      // ✅ 1. Check existing details first
      const existing = await getRentalDetailsByRentalIdAsync(rentalId);

      if (!mounted) return;

      if (existing.success && existing.data.length > 0) {
        // ✅ We have saved details → load them into rows
  ...
  isDeleted: d.isDeleted,
        ...
        setLoading(false);
        return; // ✅ STOP HERE (do not load mapping)
      }

      // ✅ 2. If no data, fallback to your existing create logic
...
const cardTitle = (row: DetailRow, indexWithinType: number) => {
  ...
  const indexWithinType = (rowIndex: number) => {
    ...
    const indices = rows
      ...
      .map(x => x.i);
    return indices.indexOf(rowIndex); // 0-based
  };
...
const handleSave = async () => {
  ...
  const isUpdate = rows.some(r => r.id !== undefined); // ✅ at least one has id
...
{cardTitle(row, indexWithinType(idx))}
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <button
          ...
          disabled={loading || rows.length === 0}
          onClick={handleSend}   // ✅ new function
        >
          Send
        </button>
